...
if ( [ -z "$MYSQL_USER" ] || [ -z "$MYSQL_PASSWORD" ] || [ -z "$MYSQL_DATABASE" ] || [ -z "$MYSQL_HOST" ] ) 
then
    echo "$STARTMSG MYSQL environment variables not there."
else
    # SLEEP 1h
    sleep 3600
    ...
    while(true)
    do
        [ -z "$(mysql -u "$MYSQL_USER" -p"$MYSQL_PASSWORD" "$MYSQL_DATABASE" -h "$MYSQL_HOST" -e 'select 1;'|tail -1|grep ERROR)" ] && [ ! -z "$($CAKE Admin getSetting MISP.baseurl|grep value)" ] && break;
        sleep 3
    done

    [ -z "$AUTH_KEY" ] && AUTH_KEY=$(mysql -u "$MYSQL_USER" -p"$MYSQL_PASSWORD" "$MYSQL_DATABASE" -h "$MYSQL_HOST" -e "SELECT authkey FROM users;" | head -2|tail -1)
